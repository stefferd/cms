...
class DaoPages implements IntDaoPages
{
    ...
    public function getMain() {
        ...
        if ($smtm = $db->mysqli->prepare('SELECT id, uniqueid, title, active, created, updated, parent FROM pages WHERE parent = 0 ORDER BY id ASC')) {
            $smtm->execute();
    ...
    public function getChildren($parent) {
        ...
        if ($smtm = $db->mysqli->prepare('SELECT id, uniqueid, title, active, created, updated, parent FROM pages WHERE parent = ? ORDER BY id ASC')) {
